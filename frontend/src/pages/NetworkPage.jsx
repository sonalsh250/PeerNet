						...
						<div className='mb-8'>
							...
							<div className='space-y-4'>
								{connectionRequests.data.map((request,index) => (
									<FriendRequest key={request.id || index} request={request} />
								))}
							</div>
						</div>
					) : (
						<div className='bg-gray-100 rounded-lg shadow p-6 text-center mb-6'>
						...
						<div className='mb-8'>
							...
							<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
								{connections.data.map((connection, index) => (
									<UserCard key={connection._id || index} user={connection} isConnection={true} />
								))}
							</div>
						</div>
					)}
